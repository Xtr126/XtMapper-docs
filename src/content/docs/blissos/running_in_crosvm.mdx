...
```
Add `--net tap-name=crosvm_tap` to the crosvm command line.
...
Add VIRT_WIFI=1 to kernel command line parameters to enable this feature.
```bash
--params "... VIRT_WIFI=1"
```
...
- crosvm crashes with virtio-gpu error: Remove `udmabuf=true` from crosvm command line, udmabuf is not supported by all GPUs.

## Venus Vulkan driver 
After applying the following patch, the embedded virglrenderer library is compiled with minigbm allocation. This also enables venus for Vulkan support which requires minigbm.
```bash
